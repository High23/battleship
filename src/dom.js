...
function mouseHover() {
    ...
    hovered.push(this);
    const coords = this.dataset.squareVertex.split('');
    const gameboardSquares = document.querySelectorAll('.gameboard-square');
    let i = 0;
    ...
    for (let j = 0; j < shipLength - 1; j++) {
        if (shipDirection === 'horizontal' && gameboardSquareElementSibling) {
            siblingCoords =
                gameboardSquareElementSibling.dataset.squareVertex.split('');
            if (
                siblingCoords[siblingCoords.length - 1] ===
                coords[coords.length - 1]
            ) {
                hovered.push(gameboardSquareElementSibling);
                if (
                    gameboardSquareElementSibling.classList[1] !==
                    'ship-placement'
                )
                    gameboardSquareElementSibling.classList.add(
                        'hover-placement',
                    );
                gameboardSquareElementSibling =
                    gameboardSquareElementSibling.nextSibling;
            }
        } else if (shipDirection === 'vertical') {
            const verticalSiblingIndex = i + 10 * (j + 1);
            if (
                verticalSiblingIndex < 100 &&
                gameboardSquares[verticalSiblingIndex].classList[1] !==
                    'ship-placement'
            ) {
                hovered.push(gameboardSquares[verticalSiblingIndex]);
                gameboardSquares[verticalSiblingIndex].classList.add(
                    'hover-placement',
                );
            }
        }
...
function mouseClick() {
    const coords = this.dataset.squareVertex.split('');
    if (coords.length > 2) {
        coords[0] += coords.splice(1, 1)[0];
    }
    coords[0] = Number(coords[0]);
    const shipPlacement = initShip(coords, shipTypes[ship], shipDirection);
    if (shipPlacement === false) {
        alert('You can not have ships overlapping one another!');
        return;
    }
    if (shipPlacement === 'invalid') {
        alert('You can not place the ship here');
        return;
    }
    ship -= 1;
    if (shipTypes[ship] !== 'submarine') shipLength -= 1;
    if (shipTypes[ship] !== undefined) {
        ...
        else displayShipName.innerHTML = shipName;
    }
...
function addAiBoardListeners() {
    const aiGameboardSquares = document.querySelectorAll(
        '.ai-gameboard-square',
    );
...
function markMissedShot(missedShot, turn) {
    ...
    for (let i = 0; i < gameboardSquares.length; i++) {
        const squareCoord = gameboardSquares[i].dataset.squareVertex.split('');
        if (squareCoord.length > 2) {
            squareCoord[0] += squareCoord.splice(1, 1)[0];
        }
        squareCoord[0] = Number(squareCoord[0]);
        if (
            squareCoord[0] === missedShot[0] &&
            squareCoord[1] === missedShot[1]
        ) {
...
function markHit(attackCoords, turn) {
    ...
    for (let i = 0; i < gameboardSquares.length; i++) {
        ...
        squareCoord[0] = Number(squareCoord[0]);
        if (
            squareCoord[0] === attackCoords[0] &&
            squareCoord[1] === attackCoords[1]
        ) {
